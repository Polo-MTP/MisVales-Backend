...
/**
 * 3 usuarios de prueba por cada rol (21 en total) para entregar a infraestructura.
 * Usa alias + de Gmail sobre jonathan.hister.6776@gmail.com, así que todos caen en
 * la misma bandeja. Idempotente y aditivo: no toca los usuarios de los demás seeders.
 */
final class JonathanUsersSeeder extends Seeder
{
        ...
        foreach (self::ROLE_CODES as $roleName => $code) {
            ...
            for ($i = 1; $i <= self::PER_ROLE; $i++) {
                ...
                $user = User::query()->updateOrCreate(
                    ['email' => $email],
                    [
                        'name' => sprintf('%s %d (QA)', $roleName, $i),
                ...
                if ($roleName === 'Distribuidora') {
                    ...
                    if (! $user->datos_id) {
                        ...
                        $datos = DatosPersonales::query()->create([
                            'nombre' => 'QA', 'apellido_paterno' => sprintf('Distribuidora %d', $i),
                            'curp' => sprintf('JON%d250101HDGZLA0%d', $i, $i), 'direccion_id' => $direccion->id,
                        ]);
